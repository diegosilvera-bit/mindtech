<?php 
require_once __DIR__ . '/../includes/functions.php'; 
require_once '../includes/auth.php'; 

// Inclui a conexão com o banco de dados
include '../config/conexao.php'; 

$mensagem = ''; 

// Verifica se o formulário foi enviado
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    
    // Pega os dados do formulário de forma simples
    $nome = $_POST['nome'];
    $cpf = $_POST['cpf'];
    $rg = $_POST['rg'];
    $telefone = $_POST['telefone'];
    $endereco = $_POST['endereco'];
    $data_nascimento = $_POST['data_nascimento'];

    // Validação básica (Nome e CPF são obrigatórios)
    if ($nome == '' || $cpf == '') {
        $mensagem = "Os campos Nome e CPF são obrigatórios.";
    } else {
        
        // Proteção contra aspas para não quebrar o banco de dados
        $nome = mysqli_real_escape_string($conn, $nome);
        $cpf = mysqli_real_escape_string($conn, $cpf);
        $rg = mysqli_real_escape_string($conn, $rg);
        $telefone = mysqli_real_escape_string($conn, $telefone);
        $endereco = mysqli_real_escape_string($conn, $endereco);
        
        // Tratamento simples para a data (se estiver vazia, grava como nulo no banco)
        if ($data_nascimento == '') {
            $sql_data = "NULL";
        } else {
            $sql_data = "'$data_nascimento'";
        }

        // Monta o comando SQL para inserir o cliente
        $sql = "INSERT INTO clientes (nome, cpf, rg, telefone, endereco, data_nascimento) 
                VALUES ('$nome', '$cpf', '$rg', '$telefone', '$endereco', $sql_data)";
        
        // Executa o comando e verifica se deu certo
        if (mysqli_query($conn, $sql)) {
            $mensagem = "Cliente cadastrado com sucesso!";
        } else {
            $mensagem = "Erro ao cadastrar o cliente: " . mysqli_error($conn);
        }
    }
}

include '../includes/header.php'; 
?>

<div class="container mt-4 mb-5"> <!-- Crentalização da tela, ajuste para não encostar na barra de cima (mt-4), (mb-5) afastada para baixo -->
    <div class="d-flex justify-content-between align-items-center mb-4"> <!-- Deixa o título e o botão "Voltar" um de cada lado, bem alinhados. -->
        
        <h1>Cadastrar Novo Cliente</h1>
        <a href="listar.php" class="btn btn-outline-secondary">Voltar para a Lista</a> <!-- Botão de voltar com borda cinza (estilo outline). -->

    </div>

    <?php if ($mensagem != '') { ?>
        <div class="alert alert-info shadow-sm"> <!-- Mensagem de feedback (sucesso ou erro) -->
            <?php echo $mensagem; ?>
        </div>
    <?php } ?>

    <div class="card shadow-sm border-0 border-start border-4 border-success"> <!-- Faz um fundo banco atrás dos campos de inserir dados -->
        <div class="card-body p-4"> <!-- Ajustamento dos campos -->
            <p class="text-muted mb-4">Preencha os dados pessoais e de contato do cliente.</p>
            
            <form method="post" action="">
                
                <div class="row"> <!-- Ajuste coloca um ao lado como excel -->
                    <div class="col-md-8 mb-3"> <!-- diz quantas partes o campo de inserir dados vai ocupar vai ocupar -->
                        <label class="form-label fw-bold">Nome Completo *</label> <!-- "form-label fw-bold" deixa o campo de inserir dados mais bonito -->
                        <input type="text" class="form-control" name="nome" placeholder="Ex: João da Silva" required>
                    </div>

                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold">Data de Nascimento</label>
                        <input type="date" class="form-control" name="data_nascimento">
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold">CPF *</label>
                        <input type="text" class="form-control" name="cpf" placeholder="000.000.000-00" required>
                    </div>

                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold">RG</label>
                        <input type="text" class="form-control" name="rg" placeholder="Ex: 12.345.678-9">
                    </div>

                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-bold">Telefone / WhatsApp</label>
                        <input type="text" class="form-control" name="telefone" placeholder="(00) 00000-0000">
                    </div>
                </div>

                <div class="mb-3 mt-2">
                    <label class="form-label fw-bold">Endereço Completo</label>
                    <input type="text" class="form-control" name="endereco" placeholder="Ex: Rua das Flores, 123 - Centro">
                </div>

                <hr class="mt-4"> <!-- Faz uma separação das linhas dos botões, definida pelo tamanho (padrão com valor 4) -->
                <div class="d-flex justify-content-end gap-2"> <!-- Deixa os dois botões no canto direito e o "gap-2" da um pequeno espaço -->
                    <a href="listar.php" class="btn btn-light border">Cancelar</a>
                    <button class="btn btn-success" type="submit">Salvar Cliente</button>
                </div>

            </form>
        </div>
    </div>
</div>

<?php include '../includes/footer.php'; ?>